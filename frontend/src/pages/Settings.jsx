...
export default function Settings() {
  ...
  const [categories, setCategories] = useState(['Single Room', 'Double Room', 'Executive Suite']);
                  ...
                  <div className="space-y-3">
                    {categories.map((category, index) => (
                      <div key={index} className="flex items-center gap-3">
                        <input value={category} onChange={(e) => updateCategory(index, e.target.value)} type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 text-sm font-medium text-slate-800 dark:text-white" />
                        <button type="button" onClick={() => removeCategory(index)} className="p-3 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl transition-colors shrink-0 font-semibold cursor-pointer">
                          Remove
                        </button>
                      </div>
                    ))}
                    <button type="button" onClick={addCategory} className="text-sm font-bold text-emerald-500 hover:text-emerald-600 dark:text-emerald-400 dark:hover:text-emerald-300 mt-2 flex items-center gap-2 cursor-pointer p-1 rounded transition-colors">
                       + Add Category
                    </button>
                  </div>
                </div>

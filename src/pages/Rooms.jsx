...
export default function Rooms() {
  ...
  const confirmDelete = () => {
    ...
    addToast(`Room ${currentRoom.id} deleted successfully.`);
  ...
  const handleSubmit = (e) => {
    ...
    if (currentRoom) {
      updateRoom(newRoom);
      addToast(`Room ${newRoom.id} updated successfully.`);
    } else {
      // Basic validation for existing room
      if (rooms.some(r => r.id === newRoom.id)) {
        addToast(`Room ${newRoom.id} already exists!`, 'error');
        return;
      }
      addRoom(newRoom);
            ...
            <tbody>
              {rooms.length === 0 ? (
                ...
                <tr key={room.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors last:border-0 group">
                  <td className="py-4 px-6 text-sm font-bold text-slate-800 dark:text-white">{room.id}</td>
                  ...
                  <td className="py-4 px-6 text-sm font-bold text-slate-800 dark:text-slate-200">${room.price}</td>
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input required defaultValue={currentRoom?.price} name="price" type="number" className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 text-sm dark:text-white" placeholder="100" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Capacity</label>
                <input required defaultValue={currentRoom?.capacity} name="capacity" type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 text-sm dark:text-white" placeholder="e.g. 2 Adults" />
              </div>
              <div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 dark:bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-[#1E293B] rounded-2xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center animate-in zoom-in-95 duration-200 border border-slate-200/60 dark:border-slate-800">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6 font-medium">Are you sure you want to delete room {currentRoom?.id}? This action cannot be undone.</p>
